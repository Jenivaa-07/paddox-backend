...
const VALID_TEAMS = [
  'Scuderia Ferrari', 'Oracle Red Bull Racing', 'Mercedes-AMG Petronas', 'McLaren F1 Team',
  'Aston Martin F1', 'BWT Alpine F1', 'Williams Racing', 'Haas F1 Team', 'RB F1 Team',
  'Audi F1 Team', 'Cadillac F1 Team'
];
...
exports.updatePreferences = async (req, res) => {
  try {
    ...
    let favouriteTeam = user.preferences?.favouriteTeam ?? '';
    if (req.body.favouriteTeam) {
      if (VALID_TEAMS.includes(req.body.favouriteTeam)) {
         favouriteTeam = req.body.favouriteTeam;
      }
    }

    let favouriteDriver = user.preferences?.favouriteDriver ?? '';
    if (req.body.favouriteDriver && typeof req.body.favouriteDriver === 'string' && req.body.favouriteDriver.length > 2) {
       // Assuming any reasonable string for driver right now, or you could strictly check all 22.
       favouriteDriver = req.body.favouriteDriver;
    }
    ...
    // TRIGGER ONBOARDING ACHIEVEMENT VIA RELIABLE OUTBOX
    if (user.preferences.favouriteTeam || user.preferences.favouriteDriver) {
      try {
        const CollectibleDefinition = require('../models/CollectibleDefinition');
        const AchievementOutbox = require('../models/AchievementOutbox');
        const onboardingDef = await CollectibleDefinition.findOne({ slug: 'onboarding-achievement', active: true });
        
        if (onboardingDef) {
          const trustedEventReference = `pref_${user._id}`;
          const eventKey = `${user._id}_${trustedEventReference}`;
          
          await AchievementOutbox.findOneAndUpdate(
             { eventKey },
             {
               $setOnInsert: {
                 eventKey,
                 userId: user._id,
                 collectibleDefinitionId: onboardingDef._id,
                 evidenceType: 'preference_save',
          ...
          // Asynchronous best-effort processor
          setTimeout(async () => {
             const { processOutboxEvents } = require('../services/outbox.processor');
             await processOutboxEvents().catch(() => {});
          }, 50);
        }
      } catch (err) {
        console.error('Achievement outbox write failed:', err.name);
      }
    }
